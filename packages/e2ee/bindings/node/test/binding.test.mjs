...
const production = await import(new URL("dist/package/loader/index.js", packageRoot));
...
async function activatedPair(seed = 20) {
  ...
  const daemon = fixture.testDaemonEndpoint(join(root, "daemon"), account, installation, session);
  const invitation = await daemon.issue(operation(1));
  const device = fixture.testDeviceEndpoint(join(root, "device"), account, installation, session, deviceId);
  ...
  assert.equal((await daemon.confirmClaim(operation(5), pending.hash, reservationId)).tag, "reserved");
  ...
  assert.rejects(device.prepareApplication(operation(10), operation(11), 1n, Buffer.from("blocked")), { code: "conflict" });
  const acceptance = await daemon.acceptActivation(operation(12), operation(9), activation.ciphertext);
  assert.equal(await device.acknowledgeActivation(operation(13), acceptance), "active");
...
test("production artifact reports its ABI and fails closed", async () => {
  ...
  await assert.rejects(production.openDeviceEndpoint(), { name: "AxlE2eeError", code: "rollback_anchor_unavailable" });
  await assert.rejects(production.openDaemonEndpoint(), { name: "AxlE2eeError", code: "rollback_anchor_unavailable" });
});
...
test("checked-in pairing fixtures are consumed and bounds precede decoding", async () => {
  ...
  const maximumClaim = readFileSync(new URL("pairing-claim-v1-maximum.tls", fixtures));
  const expected = Object.fromEntries(readFileSync(new URL("expected.txt", fixtures), "utf8").split("\n").filter((line) => line && !line.startsWith("#")).map((line) => line.split("=")));
  const invitationResult = await production.inspectPairingInvitation(invitation);
  ...
  assert.equal(createHash("sha384").update(invitation).digest("hex"), expected.invitation_sha384);
  assert.equal(createHash("sha384").update(claim).digest("hex"), expected.claim_sha384);
  assert.equal(maximumClaim.length, Number(expected.maximum_claim_bytes));
  await assert.rejects(production.inspectPairingInvitation(Buffer.alloc(2049)), { code: "bound_exceeded" });
...
test("fresh lifecycle preserves barriers, exact retries, copied input, and close semantics", async () => {
  ...
  try {
    ...
    const recovered = await pair.daemon.receiveApplication(operation(22), sent.ciphertext, operation(21), 7n);
    ...
    const deliveredPlaintext = await pair.device.receiveApplication(operation(75), delivered.ciphertext, operation(74), 7n);
    ...
    const proposal = await pair.device.prepareReplacement(operation(23), operation(24), 7n);
    await pair.daemon.receiveReplacementProposal(operation(25), proposal.ciphertext, operation(24), 7n);
    const commit = await pair.daemon.createUpdateCommit(operation(26), operation(27), 7n);
    await assert.rejects(pair.daemon.prepareApplication(operation(28), operation(29), 7n, Buffer.from("barrier")), { code: "conflict" });
    const ready = await pair.device.applyUpdateCommit(operation(30), commit, operation(27), 7n, operation(31));
    await assert.rejects(pair.device.prepareApplication(operation(32), operation(33), 7n, Buffer.from("barrier")), { code: "conflict" });
    const epochAcceptance = await pair.daemon.acceptEpochReady(operation(34), operation(31), ready.ciphertext);
    assert.equal(await pair.device.acknowledgeEpochReady(operation(35), epochAcceptance), "active");

    const retryBeforeClose = await pair.device.prepareApplication(operation(36), operation(37), 8n, Buffer.from("reopen retry"));
    ...
    const retryAfterClose = await pair.device.prepareApplication(operation(36), operation(37), 8n, Buffer.from("reopen retry"));
    assert.deepEqual(retryAfterClose.ciphertext, retryBeforeClose.ciphertext);
    await pair.device.close();
    await pair.device.close();
  } finally {
    try { await pair.daemon.close(); } catch {}
...
test("panic containment releases operation ownership", async () => {
  ...
  try {
    await assert.rejects(fixture.testPanic(pair.daemon), { code: "internal_error" });
    await assert.rejects(pair.daemon.pairStatus(), { code: "internal_error" });
  } finally {
    try { await pair.device.close(); } catch {}
    rmSync(pair.root, { recursive: true, force: true });
  }
});
...
test("daemon-only removal and device reset become terminal", async () => {
  ...
  try {
    const removal = await pair.daemon.revokeDevice(operation(80), operation(81), 9n);
    assert.equal(await pair.daemon.pairStatus(), "revoked");
    assert.equal(await pair.device.applyRemoval(operation(82), removal, operation(81), 9n), "removed");
...
test("state loss and corrupt storage fail closed", async () => {
  ...
  try { await corrupt.daemon.close(); } catch {}
