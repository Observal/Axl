...
Remote-authority infrastructure persists local device grants, intersects them with hosted narrowing grants, and enforces terminal revocation. An internal authenticated attachment maps an explicit RPC subset to `observe` or `steer` and reuses the existing dispatcher and durable command journal. `WindowsRemoteE2eeBridge` connects an injected native endpoint to this boundary: it authenticates and persists incoming MLS messages before authorization, checks the hosted grant generation, and encrypts daemon responses before opaque relay delivery. Production runtime construction remains disabled; local integration tests use the real OpenMLS binding with test-only key and rollback stores.
