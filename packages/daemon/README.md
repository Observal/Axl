...
Remote-authority infrastructure persists local device grants, intersects them with hosted narrowing grants, and enforces terminal revocation. An internal authenticated attachment maps an explicit RPC subset to `observe` or `steer` and reuses the existing dispatcher and durable command journal. It is not connected to a relay or ordinary session and does not provide cryptography. Its current integration coverage uses only the test fake E2EE adapter.
