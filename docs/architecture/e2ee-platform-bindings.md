...
The successor key record is written `prepared` in the commit transaction and marked `active` in its own strict transaction after the commit completes; open, pending, and continuation finish an interrupted activation before exposing the request. Continuation verifies the certificate in WASM against Rust-owned replica trust, rechecks the successor key, deletes the obsolete key and observes it absent, marks the operation completed, and advances the confirmed head in one strict transaction before Rust releases the exact result. After restart every retained result requires a fresh witness head before it is released.

The sealed inner state is the browser device image: identity, signer, joined-group metadata, the complete OpenMLS provider storage, the applied commit awaiting its epoch-ready message, and the completed operations with exact results inside the shared 4,096-successor idempotency horizon. The Rust `BrowserEndpoint` decodes a transient phone from it for exactly one OpenMLS transition per mutation and discards the phone whether or not the worker's commit succeeds. Supported mutations are KeyPackage creation, Welcome join, activation send, application send and receive, self-Update proposal send, received commit apply, epoch-ready send as a separate operation, epoch-ready confirmation receive, and received removal. The native-only pairing lifecycle has no browser implementation; "Browser mutation coverage" in `packages/e2ee/BROWSER_STORAGE.md` lists the deferred rows.
