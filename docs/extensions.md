...
Daemon extensions load in the `standard` tool profile. Changed source is re-imported when a session runtime reloads. The remaining browser-safe web host is tracked in the extension parity matrix.
